...
namespace X12UtilsFRM
{
    ...
    public partial class X12UtilsFRM : Form
    {
        ...
        private void pnlFunctoids_DragDrop(object sender, DragEventArgs e)
        {
            ...
            // --- 2. EXTRACT SOURCE DATA PAYLOAD (Refactored to Switch Pattern) ---
            // Determine the drag format type out of the formats collection
            string matchingFormat = null;
            if (e.Data.GetDataPresent(typeof(TreeNode))) matchingFormat = "TreeNode";
            else if (e.Data.GetDataPresent(typeof(BizTalkFunctoidNode))) matchingFormat = "BizTalkFunctoidNode";
            ...
            switch (matchingFormat)
            {
                case "TreeNode":
                    ...
                    if (sourceNode != null)
                    {
                        // Scenario A: Extract our rich layout payload object from a toolbox drag
                        if (sourceNode.Tag is ToolboxCategory.FunctoidDragData dragData)
                        {
                            string cleanName = FunctoidRegistry.NormalizeFunctoidName(sourceNode.Text);

                            // Create the visual control panel capsule
                            ...
                            // ENCAPSULATION: Assign the XML template blueprint onto the individual node property context
                            if (customFunctoidBlock is BizTalkFunctoidNode bizTalkNode)
                            {
                                var xmlConfig = FunctoidRegistry.GetConfig(cleanName);
                                if (xmlConfig != null)
                                {
                                    bizTalkNode.CustomScript = xmlConfig.ScriptTemplate;
                                }
                            }
                        ...
                        // Scenario B: It's a virtual schema node payload generated by our SkiaMapper
                ...
                default:
                    // Unrecognized drag formats exit instantly
                    return;
            }
            ...
            if (connectionSource != null)
            {
                ...
                if (targetControl != null && targetControl != connectionSource)
                {
                    ...
                    else if (targetControl is SkiaMapper && isSourceANode)
                    {
                        ...
                        // ENCAPSULATION: Pass matching built-in script logic to auto-generated fallback boxes too
                        if (autoFunctoid is BizTalkFunctoidNode bizTalkAutoNode)
                        {
                            var xmlConfig = FunctoidRegistry.GetConfig(((XmlNode)connectionSource).Name);
                            if (xmlConfig != null) bizTalkAutoNode.CustomScript = xmlConfig.ScriptTemplate;
                        }
        ...
        private void btnApplyXslt_Click(object sender, EventArgs e)
        {
           ...
           ShowFormWith(ContentFromFile(infile),ContentFromFile(lbxTargetSchema.Text)   ,ContentFromFile(outXml));
        ...
        private void btnGenerateXsltFromCanvas_Click(object sender, EventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                Logger.Error($"Failed to generate XSLT map layout :\n{ex.Message}\n{ex.InnerException}");
                tabControl1.SelectedIndex = (int)enmTabPages.parse;
        ...
        private void ShowFormWith(string inputXml, string xsltTemplate, string transformOutput)
        {
            using (Form previewForm = new Form
            {
                Text = "XSLT Transformation Workbench Pipeline",
                Width = 1100,
                Height = 750,
                StartPosition = FormStartPosition.CenterParent
            })
            {
                // 1. Create the Close Button Action Control
                Button btnClose = new Button
                {
                    Text = "Close Pipeline View",
                    DialogResult = DialogResult.Cancel,
                    Width = 150,
                    Height = 32,
                    Anchor = AnchorStyles.Bottom | AnchorStyles.Right
                };
                btnClose.Location = new Point(previewForm.ClientSize.Width - btnClose.Width - 15, 10);

                previewForm.CancelButton = btnClose;
                previewForm.AcceptButton = btnClose;

                // 2. Create the Bottom Button Bar Panel
                Panel buttonPanel = new Panel
                {
                    Dock = DockStyle.Bottom,
                    Height = 55,
                ...
                // 3. Instantiate the 3 RichTextBox Workspaces
                RichTextBox rtbInputXml = CreatePreviewRichTextBox(inputXml);
                RichTextBox rtbXslt = CreatePreviewRichTextBox(xsltTemplate);
                RichTextBox rtbOutput = CreatePreviewRichTextBox(transformOutput);

                // 4. Build the Splitter Container Framework

                // Top Splitter: Splits left and right (Vertical orientation cuts vertically)
                SplitContainer splitTopRow = new SplitContainer
                {
                    Dock = DockStyle.Fill,
                ...
                splitTopRow.Panel1.Controls.Add(CreateColumnWrapper("Source Input XML Payload", rtbInputXml));
                splitTopRow.Panel2.Controls.Add(CreateColumnWrapper("XSLT Template Definition", rtbXslt));

                // Master Splitter: Splits top and bottom (Horizontal orientation cuts horizontally)
                SplitContainer splitMaster = new SplitContainer
                {
                    Dock = DockStyle.Fill,
                ...
                splitMaster.Panel1.Controls.Add(splitTopRow); // Embed side-by-side inputs on top
                splitMaster.Panel2.Controls.Add(CreateColumnWrapper("Generated Transformed Output Target", rtbOutput)); // Big output panel on bottom

                // 5. Add controls and position splitters uniformly
                previewForm.Controls.Add(splitMaster);
                previewForm.Controls.Add(buttonPanel);

                // Set initial distances once sizes are computed by the layout engine
                previewForm.Load += (s, e) => {
                    splitMaster.SplitterDistance = (int)(previewForm.ClientSize.Height * 0.55); // Top gets 55% of height
                    splitTopRow.SplitterDistance = previewForm.ClientSize.Width / 2;           // Split evenly 50/50
                };

                // Handle button placement on window resizing
                previewForm.Resize += (s, e) => {
                    btnClose.Location = new Point(buttonPanel.Width - btnClose.Width - 15, 10);
                };
        ...
        private RichTextBox CreatePreviewRichTextBox(string textContent)
        {
        ...
        private Panel CreateColumnWrapper(string titleHeader, RichTextBox targetBox)
        {
            Panel panelWrapper = new Panel { Dock = DockStyle.Fill };

            Label lblHeader = new Label
            {
                Text = titleHeader.ToUpper(),
                Dock = DockStyle.Top,
                Height = 25,
                Font = new Font("Segoe UI", 9f, FontStyle.Bold),
                BackColor = Color.FromArgb(215, 228, 242),
                ForeColor = Color.FromArgb(30, 60, 95),
                TextAlign = ContentAlignment.MiddleLeft,
                Padding = new Padding(5, 0, 0, 0)
            };

            panelWrapper.Controls.Add(targetBox);
            panelWrapper.Controls.Add(lblHeader);
            return panelWrapper;
        }
